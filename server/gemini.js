...
const NO_KEY_MESSAGE =
  'Gemini APIキーが未登録です。ダッシュボードの「アカウント」タブで登録してください';
    ...
    actions: {
      ...
      items: {
        ...
        properties: {
          op: {
            type: 'string',
            enum: ['add_task', 'complete_task', 'delete_task', 'update_task', 'none'],
          },
          type: { type: 'string', enum: ['kadai', 'yotei'] },
          content: { type: 'string' },
          details: { type: 'string' },
...
async function ask(email, question, ctx = {}) {
  ...
  const fileIndexText = (ctx.fileIndex || [])
    .map((f) =>
      `${f.filename}${f.label ? `（${f.label}）` : ''} ${f.chars != null ? `${f.chars}字` : ''}`.trim(),
    )
    .join('\n')
    .slice(0, 8000);
  ...
  const prompt = [
    ...
    '',
    ...(ctx.targetDay
      ? [`■ ${ctx.targetDay} の授業記録（全文）:`, dayTranscriptsText || '（この日の記録なし）', '']
      : []),
    ...(fileIndexText
      ? [
      ...
      : []),
    ...(fetchedText
      ? ['■ あなたの要求（need_files）に応じて取得したファイル本文:', fetchedText, '']
      : []),
    '【利用者の発話】',
    question,
    '',
