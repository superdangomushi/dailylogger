...
const gatewayBinary =
  process.env.AIHELPER_TEST_GATEWAY_BIN || path.join(serverDir, 'aihelper-server');
let backend;
let backendPort;
let gatewayPort;
let gateway;
...
before(async () => {
  backend = http.createServer(async (request, response) => {
    ...
    if (request.url === '/no-framing') {
      response.setHeader('X-Seen-Content-Length', request.headers['content-length'] ?? 'absent');
      response.end('ok');
      return;
    }

    if (request.url === '/trailer') {
      response.setHeader('X-Seen-Authorization', request.headers.authorization ?? 'absent');
      response.end('ok');
      return;
    }
...
test('raw target, Host, auth headers, and trusted client IP reach Node', async () => {
  ...
  assert.notEqual(echoed.headers['x-aihelper-gateway-client-ip'], '203.0.113.99');
...
test('chunked request bodies are forwarded without changing bytes', async () => {
  ...
  const echoed = JSON.parse(result.body.toString('utf8'));
  assert.deepEqual(Buffer.from(echoed.bodyBase64, 'base64'), Buffer.concat(chunks));
});
...
test('ambiguous HTTP framing is rejected before reaching Node', async () => {
  ...
  const oversizedRequestLine = await rawSocketRequest(`GET /${'a'.repeat(17 * 1024)}`);
...
test('absolute read deadlines release workers from slow headers and framing', async (t) => {
  await t.test('unfinished headers', async () => {
    const attempts = await Promise.all(
      Array.from({ length: 8 }, () =>
        dripUntilClosed('GET /slow-header HTTP/1.1\r\n' + 'Host: localhost\r\n' + 'X-Slow: '),
      ),
    );
...
test('large chunked responses stream through with identical bytes', async () => {
  const expected = Buffer.concat(
    Array.from({ length: 96 }, (_, index) => Buffer.alloc(64 * 1024, index)),
  );
...
test('compressed backend responses are forwarded without decompression', async () => {
  ...
  assert.deepEqual(zlib.gunzipSync(result.body), Buffer.from('{"from":"backend"}'));
});
