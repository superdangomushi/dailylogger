...
function rateLimit({ windowMs, max, keyPrefix, keyOf }) {
  ...
  return (req, res, next) => {
    ...
    if (current.count > max) {
      ...
      return res
        .status(429)
        .json({ ok: false, error: 'リクエストが多すぎます。少し待ってから再試行してください' });
    }
    next();
  };
}
...
function handleBadGeminiKey(res, e) {
  ...
  res.status(400).json({
    ok: false,
    error:
      '登録されている Gemini APIキーが無効になっています。「アカウント」タブで登録し直してください',
  });
  return true;
}
...
app.post('/api/login', authLimiter, async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      email: account.email,
      token: account.token,
      line: Boolean(account.lineUserId),
    });
  } catch (e) {
...
app.post('/api/change-password', async (req, res) => {
  ...
  try {
    ...
    if (!u) {
      return res
        .status(400)
        .json({ ok: false, error: 'このアカウントはパスワード変更に対応していません' });
    }
...
app.patch('/api/courses/:id', async (req, res) => {
  ...
  if (!String(name || '').trim())
    return res.status(400).json({ ok: false, error: '科目名が必要です' });
  try {
    const ok = await db.updateCourse(account.email, req.params.id, {
      term,
      day,
      period,
      name,
...
app.post('/api/files', heavyLimiter, async (req, res) => {
  ...
  try {
    ...
    if (ctype.startsWith('text/plain')) {
      ...
      return res
        .status(400)
        .json({ ok: false, error: 'ファイル本文がありません（PDF か .txt を送ってください）' });
    }
...
function googleRedirectUri(req) {
  ...
  if (allow.length && !allow.includes(host)) {
    throw new Error(
      'この Host からの Google 連携は許可されていません（GOOGLE_REDIRECT_URL を設定してください）',
    );
  }
...
app.get('/api/google/callback', async (req, res) => {
  ...
  try {
    ...
    const { googleEmail, refreshToken } = await google.exchangeCode(
...
app.get('/api/google/accounts', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      accounts: rows.map((r) => r.google_email),
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: serverErr(e) });
  }
...
app.post('/api/google/add-event', async (req, res) => {
  ...
  try {
    ...
    await google.insertDeadline(
      token,
      content,
      String(req.body?.deadline || ''),
      Boolean(req.body?.dateOnly),
    );
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ ok: false, error: serverErr(e) });
...
app.post('/api/google/sync-courses', async (req, res) => {
  ...
  try {
    ...
    const untilStr = isSpring
      ? `${year}0731T235959Z`
      : `${year + (month <= 3 ? 0 : 1)}0131T235959Z`;
...
app.post('/api/stt-quality', async (req, res) => {
  ...
  if (!STT_QUALITIES.includes(quality)) {
    return res
      .status(400)
      .json({ ok: false, error: 'quality は light/standard/high のいずれかを指定してください' });
  }
  try {
...
app.post('/api/gemini-key', async (req, res) => {
  ...
  try {
    ...
    if (!check.ok) {
      return res
        .status(400)
        .json({ ok: false, error: `APIキーの確認に失敗しました: ${check.error}` });
    }
    ...
    if (!n) {
      ...
      return res.status(400).json({
        ok: false,
        error:
          'このアカウントにはAPIキーを保存できません（Web登録のアカウントでログインしてください）',
      });
    }
  ...
  try {
    ...
    if (!n) {
      return res.status(400).json({
        ok: false,
        error:
          'このアカウントでは設定を保存できません（Web登録のアカウントでログインしてください）',
      });
    }
    res.json({ ok: true, enabled });
  } catch (e) {
...
app.get('/api/waseda/credentials', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ok: true,
      wasedaUser: row.waseda_user,
      wasedaPassword: decryptCred(row.waseda_password_enc),
    });
  } catch (e) {
...
app.post('/api/audio', heavyLimiter, async (req, res) => {
  ...
  if (!Buffer.isBuffer(req.body) || req.body.length === 0) {
    return res
      .status(400)
      .json({ ok: false, error: '音声データがありません（audio/wav 等で送ってください）' });
  }
...
app.get('/api/audio/jobs', async (req, res) => {
  ...
  try {
    ...
    const jobs = await db.listAudioJobs(account.email, {
    ...
    res.json({
      ...
      workers: workers.map((w) => {
        ...
        return {
          ...
          online:
            Boolean(w.last_seen_at) && Date.now() - new Date(w.last_seen_at).getTime() < 60_000,
          ...
          metricsFresh:
            Boolean(w.metrics_at) && Date.now() - new Date(w.metrics_at).getTime() < 15_000,
        };
      }),
    });
  } catch (e) {
...
app.post('/api/audio/workers/:id', async (req, res) => {
  ...
  if (!Number.isInteger(id) || id <= 0)
    return res.status(400).json({ ok: false, error: 'IDが不正です' });
...
function clientIdFromBody(req) {
  const raw = String(req.body?.clientId || '')
    .trim()
    .toLowerCase();
...
async function requireClientWorker(req, res) {
  ...
  if (!worker) {
    res.status(403).json({
      ...
      code: 'unregistered',
      error:
        'このPCは未登録です。クライアントの初回登録（/api/client/register）をやり直してください',
    });
    return null;
  }
...
app.post('/api/client/register', async (req, res) => {
  ...
  const clientId = clientIdFromBody(req);
  if (!clientId)
    return res.status(400).json({ ok: false, error: 'clientId（UUID形式）を指定してください' });
  const name = String(req.body?.name || '')
    .trim()
    .slice(0, 255);
  if (!name)
    return res.status(400).json({ ok: false, error: 'このPCの表示名（name）を指定してください' });
  const mode = req.body?.mode === 'global' ? 'global' : 'private';
  try {
    const worker = await db.registerAudioWorker(account.email, clientId, {
      name,
      mode,
      ip: clientIpOf(req),
    });
    ...
    console.log(
      `ワーカー登録: ${account.email} clientId=${clientId} name=${name} (${worker.mode || mode})`,
    );
    res.json({
      ok: true,
      client: {
...
app.post('/api/client/claim', async (req, res) => {
  try {
    ...
    const declaredMode =
      req.body?.mode === 'global' || req.body?.mode === 'private' ? req.body.mode : null;
...
app.post('/api/client/jobs/download', async (req, res) => {
  try {
    ...
    if (!job)
      return res.status(404).json({ ok: false, error: '処理中の音声ジョブが見つかりません' });
...
app.post('/api/ask', askLimiter, async (req, res) => {
  ...
  try {
      ...
      for (const r of gAccounts) {
        try {
          ...
          for (const ev of gEvents) {
            ...
            const exists = calendar.some(
              (x) =>
                x.title === ev.title && Math.abs((x.startMillis || 0) - ev.startMillis) < 60000,
            );
            if (!exists) {
              calendar.push({
                title: ev.title,
    ...
    const snippets = await db.searchTranscriptSnippets(
      account.email,
      extractKeywords(question, courses),
    );
    ...
    const claimsAdd =
      /(登録|追加)(し|いたし)(ました|ます|とき|とく|ておき)|入れ(て|と)(おき|き)?ま(した|す)|入れました/.test(
        reply,
      );
    const asksAdd =
      /(入れて|入れといて|入れとって|入れておいて|(追加|登録)し(て|といて|ておいて)|リマインドして)/.test(
        question,
      );
...
function extractDateFromQuestion(question, today) {
  ...
  if (m)
    return `${base.getFullYear()}-${String(m[1]).padStart(2, '0')}-${String(m[2]).padStart(2, '0')}`;
...
function normalizeDeadlineInput(s) {
  ...
  if (dt)
    return {
      at: `${dt[1]}-${dt[2]}-${dt[3]} ${dt[4].padStart(2, '0')}:${dt[5]}:00`,
      dateOnly: false,
    };
...
async function main() {
  ...
  app.listen(PORT, HOST, () => {
    console.log(
      `${USING_CPP_GATEWAY ? 'AIHelper Node backend' : 'AIHelper'} listening on http://${HOST || 'localhost'}:${PORT}`,
    );
    console.log(`accounts: ${ACCOUNTS_FILE}`);
    console.log(`DB: ${process.env.DB_NAME || 'aihelper'}@${process.env.DB_HOST || 'localhost'}`);
    console.log(
      `Gemini: ユーザーごとのAPIキー登録制 (モデル ${gemini.MODEL}) / LINE: ${line.isConfigured() ? '有効' : '未設定'}`,
    );
    scheduleDailySummaryPregeneration();
