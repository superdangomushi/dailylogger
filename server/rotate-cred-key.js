...
async function main() {
  ...
  try {
    ...
    for (const r of wrows) {
      ...
      if (next && !DRY_RUN) {
        await pool.query('UPDATE users SET waseda_password_enc = ? WHERE email = ?', [
          next,
          r.email,
        ]);
      }
    ...
    for (const r of krows) {
      ...
      if (next && !DRY_RUN) {
        await pool.query('UPDATE users SET gemini_api_key_enc = ? WHERE email = ?', [
          next,
          r.email,
        ]);
      }
    ...
    if (DRY_RUN) console.log('--dry-run のため DB は変更していません。');
      ...
      console.log(
        '完了。サーバーには新しい CRED_ENC_KEY だけを渡し、旧 .cred-key は破棄してください。',
      );
  } finally {
    await pool.end();
  }
