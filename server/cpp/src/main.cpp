...
constexpr const char* kNoAcceptRangesMarker = "X-AIHelper-Internal-No-Accept-Ranges";
constexpr const char* kNoContentTypeMarker = "X-AIHelper-Internal-No-Content-Type";
constexpr const char* kNoContentLengthMarker = "X-AIHelper-Internal-No-Content-Length";
...
bool valid_env_name(const std::string& name) {
  if (name.empty() || !(name.front() == '_' || (name.front() >= 'A' && name.front() <= 'Z') ||
...
long env_long(const char* name, long fallback, long minimum, long maximum) {
  ...
  if (errno != 0 || end == value || *end != '\0' || parsed < minimum || parsed > maximum) {
    std::cerr << name << " が不正なため既定値 " << fallback << " を使用します: " << value
              << std::endl;
    return fallback;
  }
  return parsed;
...
class TemporaryFile {
 public:
  static std::shared_ptr<TemporaryFile> create() {
    ...
    return std::shared_ptr<TemporaryFile>(new TemporaryFile(fd, std::move(file_path)));
  }
  ...
  ssize_t read_at(size_t offset, char* data, size_t length) const {
    for (;;) {
      const auto result = ::pread(fd_, data, length, static_cast<off_t>(offset));
      if (result < 0 && errno == EINTR) {
        continue;
      }
      return result;
 ...
 private:
  TemporaryFile(int fd, std::string path) : fd_(fd), path_(std::move(path)) {}
...
std::unordered_set<std::string> connection_tokens(const httplib::Headers& headers) {
  ...
  for (auto it = range.first; it != range.second; ++it) {
    ...
    while (start <= value.size()) {
      ...
      if (!token.empty()) {
        result.insert(ascii_lower(token));
      }
...
bool is_hop_by_hop(const std::string& name, const std::unordered_set<std::string>& tokens) {
  const auto lower = ascii_lower(name);
  static const std::unordered_set<std::string> fixed{"connection",
                                                     "keep-alive",
                                                     "proxy-authenticate",
                                                     "proxy-authorization",
                                                     "proxy-connection",
...
bool is_internal_header(const std::string& name) {
  const auto lower = ascii_lower(name);
  return lower == ascii_lower(kClientIpHeader) || lower == ascii_lower(kNoAcceptRangesMarker) ||
         lower == ascii_lower(kNoContentTypeMarker) ||
         lower == ascii_lower(kNoContentLengthMarker) || lower == "remote_addr" ||
         lower == "remote_port" || lower == "local_addr" || lower == "local_port";
}
...
void publish_headers(const std::shared_ptr<ProxyState>& state, const httplib::Response& response) {
  {
...
bool enqueue_response_data(const std::shared_ptr<ProxyState>& state, const char* data,
                           size_t length) {
  std::unique_lock<std::mutex> lock(state->mutex);
  state->changed.wait(
      lock, [&] { return state->cancelled || state->queued_bytes < state->max_queued_bytes; });
  if (state->cancelled) {
    return false;
  }
...
bool write_next_response_chunk(const std::shared_ptr<ProxyState>& state, httplib::DataSink& sink,
                               bool chunked) {
  std::string data;
  {
    std::unique_lock<std::mutex> lock(state->mutex);
...
std::optional<size_t> response_content_length(const httplib::Headers& headers) {
...
size_t request_body_limit(const httplib::Request& request, size_t global_limit) {
  ...
  if (media_type == "application/pdf" || media_type == "application/octet-stream") {
 ...
 public:
  ...
  void proxy(const httplib::Request& incoming, httplib::Response& outgoing,
    ...
    for (const auto& header : incoming.headers) {
      ...
      if (is_hop_by_hop(header.first, request_tokens) || is_internal_header(header.first) ||
          lower == "content-length" || lower == "expect") {
        continue;
      }
    ...
    upstream.headers.emplace(kClientIpHeader, client_ip.empty() ? "unknown" : client_ip);

    const bool incoming_body_framed =
        incoming.has_header("Content-Length") || incoming.has_header("Transfer-Encoding");
    if (body && body->size() > 0) {
      upstream.content_length_ = body->size();
      upstream.is_chunked_content_provider_ = false;
      upstream.content_provider_ = [body](size_t offset, size_t length,
    ...
    state->client = std::make_shared<httplib::Client>(config_.backend_host, config_.backend_port);
    ...
    upstream.content_receiver = [state](const char* data, size_t length, uint64_t, uint64_t) {
    ...
    state->worker = std::thread([state, upstream = std::move(upstream)]() mutable {
      {
        std::lock_guard<std::mutex> lock(state->mutex);
        if (state->cancelled) {
          state->done = true;
      ...
      }
      {
        ...
        state->failed = !ok;
        if (!ok) {
          state->error = httplib::to_string(error);
        }
      }
      state->changed.notify_all();
    });
    ...
    {
      std::unique_lock<std::mutex> lock(state->mutex);
      state->changed.wait(lock, [&] { return state->headers_ready || state->done; });
      if (state->headers_ready) {
    ...
    if (upstream_status < 0) {
      ...
      finish_proxy(state);
      std::cerr << "Node backend への接続に失敗しました" << (reason.empty() ? "" : ": " + reason)
                << std::endl;
    ...
    const bool had_content_type = upstream_headers.find("Content-Type") != upstream_headers.end();
    const bool had_accept_ranges = upstream_headers.find("Accept-Ranges") != upstream_headers.end();
    ...
    for (const auto& header : upstream_headers) {
      const auto lower = ascii_lower(header.first);
      if (is_hop_by_hop(header.first, response_tokens) || is_internal_header(header.first) ||
    ...
    if (has_body && content_length && *content_length == 0) {
      ...
      outgoing.content_provider_ = [state, chunked = !content_length](size_t, size_t,
    ...
    outgoing.content_provider_resource_releaser_ = [state](bool) { finish_proxy(state); };
  }
...
void spool_and_proxy(const HttpGateway& gateway, const httplib::Request& request,
                     httplib::Response& response, const httplib::ContentReader& reader,
                     size_t body_limit, long body_timeout_sec) {
  ...
  const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(body_timeout_sec);
  bool read_ok = false;
  {
...
pid_t spawn_node(int backend_port) {
  ...
  ::execlp(node.c_str(), node.c_str(), entry.c_str(), static_cast<char*>(nullptr));
  std::cerr << "Node.js を起動できません: " << std::strerror(errno) << std::endl;
  ::_exit(127);
}

bool wait_for_backend(int port, pid_t child, long timeout_ms, bool& child_reaped,
                      int& child_status) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (::chdir(server_dir.c_str()) != 0) {
      std::cerr << "サーバーディレクトリへ移動できません: " << server_dir << ": "
                << std::strerror(errno) << std::endl;
      return 1;
    }
    load_dot_env(server_dir / ".env");

    const int public_port =
        static_cast<int>(env_long("PORT", 3000, 1, std::numeric_limits<uint16_t>::max()));
    ...
    int backend_port = static_cast<int>(
        env_long("AIHELPER_NODE_PORT", 0, 0, std::numeric_limits<uint16_t>::max()));
    if (backend_port == 0) {
    ...
    gateway_config.connect_timeout_sec = env_long("AIHELPER_NODE_CONNECT_TIMEOUT_SEC", 5, 1, 3600);
    ...
    gateway_config.queue_bytes = static_cast<size_t>(
        env_long("AIHELPER_HTTP_QUEUE_BYTES", 4 * 1024 * 1024, 64 * 1024, 256 * 1024 * 1024));
...
#ifdef CPPHTTPLIB_ABSOLUTE_READ_DEADLINE
    server.set_header_read_timeout(env_long("AIHELPER_HTTP_HEADER_TIMEOUT_SEC", 60, 1, 3600), 0);
#endif
    server.set_read_timeout(env_long("AIHELPER_HTTP_READ_TIMEOUT_SEC", 60, 1, 86400), 0);
    server.set_write_timeout(env_long("AIHELPER_HTTP_WRITE_TIMEOUT_SEC", 3600, 1, 86400), 0);
    const auto global_body_limit = static_cast<size_t>(
        env_long("AIHELPER_HTTP_MAX_BODY_BYTES", 300L * 1024 * 1024, 1, 2L * 1024 * 1024 * 1024));
    const auto body_timeout_sec = env_long("AIHELPER_HTTP_BODY_TIMEOUT_SEC", 300, 1, 86400);
    server.set_payload_max_length(global_body_limit);
    server.set_tcp_nodelay(true);
    const auto thread_count = static_cast<size_t>(env_long("AIHELPER_HTTP_THREADS", 32, 2, 512));
    const auto max_queued_requests =
        static_cast<size_t>(env_long("AIHELPER_HTTP_MAX_QUEUED_REQUESTS", 256, 1, 8192));
    server.new_task_queue = [thread_count, max_queued_requests] {
      return new httplib::ThreadPool(thread_count, max_queued_requests);
    };

    server.set_header_writer([](httplib::Stream& stream, httplib::Headers& headers) {
    ...
    server.set_pre_routing_handler(
        [](const httplib::Request& request, httplib::Response& response) {
          static const std::set<std::string> supported{"GET",   "HEAD",   "POST",   "PUT",
    ...
    const auto body_handler = [&gateway, global_body_limit, body_timeout_sec](
                                  const httplib::Request& request, httplib::Response& response,
                                  const httplib::ContentReader& reader) {
      spool_and_proxy(gateway, request, response, reader,
                      request_body_limit(request, global_body_limit), body_timeout_sec);
    };
    ...
    const auto bodyless_handler = [&gateway](const httplib::Request& request,
                                             httplib::Response& response) {
    ...
    if (!server.bind_to_port(public_host, public_port)) {
      std::cerr << "公開HTTPポートを開けません: " << public_host << ':' << public_port << std::endl;
      return 1;
    }
    ...
    if (!no_spawn) {
      ...
      if (child < 0) {
        std::cerr << "Node.js プロセスを作成できません: " << std::strerror(errno) << std::endl;
        return 1;
      }
    }

    const long start_timeout = env_long("AIHELPER_NODE_START_TIMEOUT_MS", 30000, 100, 300000);
    if (!wait_for_backend(backend_port, child, start_timeout, child_reaped, child_status)) {
      if (g_signal != 0) {
        terminate_child(child, child_reaped);
        return 0;
      }
      std::cerr << "Node backend が起動しませんでした (127.0.0.1:" << backend_port << ')'
                << std::endl;
      terminate_child(child, child_reaped);
      return 1;
    }
    ...
    const auto display_host =
        public_host.find(':') == std::string::npos ? public_host : '[' + public_host + ']';
    std::cout << "C++ HTTP gateway listening on http://" << display_host << ':' << public_port
              << std::endl;
      ...
      while (!stop_monitor.load()) {
        ...
        if (child > 0 && !child_reaped) {
          ...
          if (result == child) {
            ...
            ::kill(-child, SIGTERM);
            std::cerr << "Node backend が終了したためゲートウェイを停止します" << std::endl;
            server.stop();
            gateway.stop_all();
            return;
          }
    ...
    std::cerr << "HTTPゲートウェイの初期化に失敗しました: " << error.what() << std::endl;
    return 1;
  }
}
