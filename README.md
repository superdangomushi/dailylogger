...
`server/`（C++ HTTP ゲートウェイ + Node.js / Express + MySQL）。公開ポートの HTTP 通信は C++ が受け、
既存の API・ダッシュボード・業務ロジックは内部の Node.js がそのまま処理する。
受け取った音声をキュー化し、`client/`のPCワーカーから返ってきたテキストを MySQL に保存する。
...
```bash
cd server
make install                                # C++/Node.js/MySQL の準備とビルド
LINE_CHANNEL_ACCESS_TOKEN=yyyy make run     # http://localhost:3000
```
