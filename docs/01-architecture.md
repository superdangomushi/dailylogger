...
1. スマホアプリ、ブラウザ、ワーカーPCは、従来どおり `PORT`（既定3000）へアクセスする。
2. C++ ゲートウェイがメソッド、パス、クエリ、ヘッダー、本文を内部の Node.js / Express へ転送する。
3. Express が既存のAPI処理または `dashboard.js` のUI生成を行い、ゲートウェイがステータス、ヘッダー、本文を呼び出し元へ返す。

Node.js の待受はループバック内に閉じ、C++ ゲートウェイが子プロセスの起動・終了も管理する。
この分離は HTTP 通信層だけの変更で、API仕様やブラウザUIは変更しない。
